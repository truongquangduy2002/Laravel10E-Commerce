...
class SliderController extends Controller
{
    ...
    public function StoreSlider(Request $request)
    {
        ...
        if ($request->hasFile('slider_image')) {
            ...
            $uploadDirectory = public_path('upload'); // Đường dẫn thư mục upload

            $sliderDirectory = $uploadDirectory . '/sliders'; // Đường dẫn thư mục slider

            $files = File::files($sliderDirectory);

            if (count($files) === 0) {
                // Không tìm thấy ảnh trong thư mục upload/sliders
                echo "Không tìm thấy ảnh trong thư mục upload/sliders.";
            } else if (count($files) > 1) {
                // Có nhiều hơn một file trong thư mục upload
                echo "Có nhiều hơn một file trong thư mục upload/sliders.";
            } else if (File::exists($sliderDirectory)) {
                $files = File::allFiles($sliderDirectory);
                File::delete($files);
                // Xóa thành công, ảnh duy nhất trong thư mục upload biến mất
            }
        }
